...
import { Home, Users, CalendarDays, LogOut, Menu, X } from "lucide-react";
...
const navItems = [
  { href: "/prof", label: "Início", icon: Home, exact: true },
  { href: "/prof/turmas", label: "Minhas Turmas", icon: Users },
  { href: "/prof/horario", label: "Meu Horário", icon: CalendarDays },
];

export function ProfSidebar({ user }: { user: { name: string; tenantId: string } }) {
  ...
  const sidebar = (
    <aside aria-label="Menu lateral" className="flex h-full w-56 flex-col border-r border-navy-100 bg-white">
      <div className="border-b border-navy-100 px-4 py-4">
        ...
        <p className="text-xs text-teal-600 font-medium mt-0.5">Portal do Professor</p>
      </div>
      <nav aria-label="Navegação do professor" className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        {navItems.map((item) => {
      ...
      <div className="border-t border-navy-100 px-4 py-3">
        <p className="truncate text-sm font-medium text-navy-800 mb-2">{user.name}</p>
        <Button
          variant="ghost"
          size="sm"
          className="w-full justify-start gap-2 text-navy-500 hover:text-red-600 px-0"
